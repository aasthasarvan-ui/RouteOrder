...
# ==============================================================================
# 🚀 ULTIMATE CHECKBOX + DROPDOWN ADD-ON MODULE: MULTI-TRIP MERGE & EXACT SEQUENCE
...
def render_triple_mode_trip_addon(df_input):
    with st.expander("🎛️ Advanced Multi-Trip Merge & Exact Sequence Engine (Click to Expand)", expanded=True):
        st.markdown("*(Independent Module: Automatically detects trips, supports working Checkboxes & Multi-select Dropdown, and updates calculations instantly).*")
        ...
        with col_v1:
            search_v = st.text_input("🔎 [Add-on] Quick Search Vehicle (Last 4 digits e.g. 0440, 2680):", "", key="combo_addon_search")
            filtered_v = [v for v in all_vehicles if search_v.lower() in v.lower()] if search_v.strip() else all_vehicles
            sel_v = st.selectbox("🚛 Select Vehicle:", options=filtered_v if filtered_v else all_vehicles, key="combo_addon_veh_sel")
        ...
        # SELECTION MODE (Checkboxes or Multi-Select Dropdown)
        selection_mode = st.radio(
            "Choose Selection Style:",
            ["Checkboxes (Multi-Trip)", "Dropdown (Multi-Select)"],
            horizontal=True,
            key=f"combo_mode_{sel_v}"
        )

        chosen_trips = []
        all_t_names = list(trips_map.keys())

        if "Checkboxes" in selection_mode:
        ...
        bill_multiselect_key = f"combo_exact_bills_{sel_v}"
        
        final_selected_bills = st.multiselect(
            "Exact Bills Included (Fully Editable):",
            options=bills,
            default=default_active_bills,
