        ...
        with st.spinner("⚡ Running Vehicle Capacity Clubbing & Shortfall Pending Calculations... Please wait."):
            try:
                ...
                for uploaded_file in uploaded_inputs:
                    ...
                    for rSearch in range(min(10, df_input.shape[0])):
                        for cSearch in range(df_input.shape[1]):
                            ...
                            if "CONTACT" in upper_cell or "MOBILE" in upper_cell or "PHONE" in upper_cell:
                                for cOff in range(cSearch, min(df_input.shape[1], cSearch + 3)):
                                    ...
                                    if re.search(r'\d{10}', cand):
                                        file_contact_no = re.search(r'\d{10}', cand).group(0)
                                        break
                            if "VEHICLE" in upper_cell or "TRUCK" in upper_cell or "GAADI" in upper_cell:
                                for cOff in range(cSearch, min(df_input.shape[1], cSearch + 3)):
                                    cand = str(df_input.iloc[rSearch, cOff]).strip()
                                    if len(cand) > 3 and cand.upper() != "NAN":
                                        file_vehicle_no = cand
                                        break
                    ...
                    assigned_vehicle_no = file_vehicle_no
                    ...
                    for r in range(fg_row + 1, df_input.shape[0]):
                        ...
                        for _, fgc, d_dem, d_disp, d_pend in valid_row_quantities:
                            dispatch_plan_records.append((today_date, str(route_num), assigned_vehicle_no, file_contact_no, str(agency_val), fgc, d_dem, d_disp, d_pend, status_str, batch_ts))
                    ...
                    if valid_items_created > 0 or len(file_comparison_rows) > 0:
                        ...
                        buf_valid.seek(0)
                        out_fname = safe_route_num + "_" + today_date + "_" + timestamp + "_Dispatch.xlsx"
...
if st.session_state.processed_files or st.session_state.skipped_rows_log:
    ...
    # --- Unique Keys for Email Inputs to avoid DuplicateElementId ---
    with st.expander("✉️ Advanced Email Dispatch Options"):
        email_subject_val = st.text_input("Custom Email Subject Line", f"🚀 Sales Orders Dispatch Hub Report - {get_ist_now().strftime('%Y-%m-%d')}", key="unique_email_subj_key")
        email_notes_val = st.text_area("Custom Remarks / Notes", "All routes verified and processed successfully.", key="unique_email_notes_key")
    ...
    with col_zip:
        st.download_button(label="📦 ZIP", data=zip_buffer.getvalue(), file_name=f"Batch_Orders_{get_ist_now().strftime('%Y-%m-%d')}.zip", mime="application/zip", key="zip_dl")
    with col_pdf:
        try:
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", "B", 16)
            pdf.cell(190, 10, "Enterprise Dispatch Summary", ln=True, align="C")
            pdf_bytes = bytes(pdf.output())
            st.download_button(label="📄 PDF", data=pdf_bytes, file_name="Summary.pdf", mime="application/pdf", key="pdf_dl")
        except:
            pass
    with col_summary:
        st.download_button(label="📄 TXT", data=f"Total Qty: {kpi['input_qty']}".encode('utf-8'), file_name="Summary.txt", mime="text/plain", key="txt_dl")
    with col_json:
        st.download_button(label="💾 JSON", data=json.dumps(kpi).encode('utf-8'), file_name="Audit.json", mime="application/json", key="json_dl")
    with col_print:
        components.html('<div style="width:100%;"><button onclick="parent.window.print()" style="width:100%; height:38px; background:#2563eb; color:white; border:none; border-radius:4px; font-weight:600; cursor:pointer;">🖨️ Print</button></div>', height=50)
    with col_email:
        if st.button("📧 Email", key="email_btn_main"):
            if email_user and email_pass and recipient_email:
                try:
                    msg = EmailMessage()
                    msg['Subject'] = email_subject_val
                    msg['From'] = email_user
                    msg['To'] = recipient_email
                    msg.set_content(email_notes_val)
    ...
    with col_wa:
        if whatsapp_num:
            st.markdown(f'<a href="https://wa.me/{whatsapp_num}?text=Dispatch%20Ready" target="_blank"><button style="width:100%; height:38px; background:#25D366; color:white; border:none; border-radius:4px; font-weight:600;">📱 WhatsApp</button></a>', unsafe_allow_html=True)

# ==========================================
# 8. UNIVERSAL DATABASE MANAGEMENT & SCHEMA EDITOR
# ==========================================
st.markdown("---")
with st.expander("🗄️ View, Export & Manage All Databases (Add/Delete Columns & Rows)"):
    try:
        ...
        def render_table_manager(table_name, df_data):
            ...
            with col_ed1:
                new_col = st.text_input(f"New Col Name", key=f"col_{table_name}")
                if st.button(f"➕ Add Col", key=f"add_{table_name}"):
                    if new_col:
                        conn_c = sqlite3.connect("sales_history.db")
                        conn_c.execute(f"ALTER TABLE {table_name} ADD COLUMN {new_col} TEXT")
                        conn_c.commit()
                        conn_c.close()
                        st.success(f"✅ Added {new_col}!")
                        st.rerun()
            with col_ed2:
                row_id = st.number_input(f"Record ID", min_value=1, step=1, key=f"id_{table_name}")
                if st.button(f"🗑️ Delete Row", key=f"del_{table_name}"):
                    conn_r = sqlite3.connect("sales_history.db")
                    conn_r.execute(f"DELETE FROM {table_name} WHERE id = ?", (row_id,))
                    conn_r.commit()
                    conn_r.close()
                    st.success(f"✅ Deleted ID {row_id}!")
                    st.rerun()
            with col_ed3:
                if st.button(f"🚨 Wipe Table", key=f"wipe_{table_name}", type="secondary"):
                    conn_w = sqlite3.connect("sales_history.db")
                    conn_w.execute(f"DELETE FROM {table_name}")
                    conn_w.commit()
                    conn_w.close()
        ...
        with tab_m7:
            if not df_dispatch.empty:
                ...
                buf = io.BytesIO()
                df_dispatch.to_excel(buf, index=False, sheet_name="Monthly Dispatch Master")
                buf.seek(0)
                st.download_button("📥 Export Monthly Masterfile", buf.getvalue(), f"Monthly_Dispatch_{get_ist_now().strftime('%Y-%m')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_m_master")
        with tab_m8:
            if not df_dispatch.empty:
                ...
                if not df_pend.empty:
                    ...
                    buf_p = io.BytesIO()
                    df_pend.to_excel(buf_p, index=False, sheet_name="Master Pending")
                    buf_p.seek(0)
                    st.download_button("📥 Export Master Pending File", buf_p.getvalue(), f"Master_Pending_{get_ist_now().strftime('%Y-%m-%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_m_pending")
        with tab_m9:
            st.markdown("#### 🛠️ Drop Column Tool")
            sel_t = st.selectbox("Table", ['unique_routes_master', 'unmapped_missing_dr_ledger', 'output_files_ledger', 'dispatch_planning_ledger', 'inventory_stock_table'], key="drop_tbl_sel")
            conn_s = sqlite3.connect("sales_history.db")
            cur_s = conn_s.cursor()
            cur_s.execute(f"PRAGMA table_info({sel_t})")
            t_cols = [c[1] for c in cur_s.fetchall()]
            conn_s.close()
            drop_col = st.selectbox("Column", t_cols, key="drop_col_sel")
            if st.button("🗑️ Drop Column", key="drop_col_btn"):
                conn_d = sqlite3.connect("sales_history.db")
                cur_d = conn_d.cursor()
                rem = [c for c in t_cols if c != drop_col and c != 'id']
                col_str = ", ".join(rem)
                cur_d.execute(f"CREATE TABLE {sel_t}_temp AS SELECT id, {col_str} FROM {sel_t}")
                cur_d.execute(f"DROP TABLE {sel_t}")
                cur_d.execute(f"ALTER TABLE {sel_t}_temp RENAME TO {sel_t}")
                conn_d.commit()
                conn_d.close()
                st.success(f"✅ Dropped {drop_col}!")
                st.rerun()
    except Exception as e:
        st.error(f"Error: {str(e)}")
