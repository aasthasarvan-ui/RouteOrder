...
# SQLite Database Initialization with Auto-Migration for File Tracking & Output Ledger
def init_db():
    ...
    if uploaded_inputs:
        ...
        with st.spinner("⚡ Reading files, auto-looking up missing DRs from Master DB, and archiving outputs... Please wait."):
            try:
                ...
                # --- Update Master DB & Output Files Ledger (No Duplicates) ---
                conn = sqlite3.connect("sales_history.db")
                cursor = conn.cursor()
                cursor.executemany("""
...
# --- UNIQUE MASTER DATABASE MANAGEMENT & OUTPUT ARCHIVE PANEL ---
st.markdown("---")
with st.expander("🗄️ View, Export & Manage Unique Master Database & Output File Archive"):
    st.markdown("Yahan aap master database records dekh sakte hain, archived output files ko download kar sakte hain, aur file-wise, route-wise ya row-wise data delete kar sakte hain.")
    try:
        conn = sqlite3.connect("sales_history.db")
        ...
        tab_m1, tab_m2 = st.tabs(["📋 Route-Agency-DR Master Links", "📦 Archived Output Files Ledger"])
        ...
        with tab_m2:
            ...
            if not df_outputs.empty:
                ...
                selected_output_id = st.number_input("Enter Output File ID to Download from Archive", min_value=1, step=1, key="out_file_id")
                if st.button("📥 Download Archived File"):
                    ...
                    if row_res:
                        ...
                        st.download_button(
                            label=f"💾 Click here to download '{fname_res}'",
                            data=fdata_res,
                            file_name=fname_res,
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        ...
                        st.warning("⚠️ Invalid ID or File not found in archive.")
            else:
