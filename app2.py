...
# ==============================================================================
# SECTION 2: 8 ENTERPRISE THEME PALETTES & THEME ENGINE
# ==============================================================================
...
# ==============================================================================
# SECTION 3: SESSION STATE INITIALIZATION & DEFAULTS
# ==============================================================================
...
for default_key, default_val in GLOBAL_DEFAULTS.items():
...
# ==============================================================================
# SECTION 4: MODULAR DATABASE INITIALIZATION & SCHEMA DEFINITIONS
# ==============================================================================
...
def init_all_enterprise_databases():
    ...
    # 10. Generated Binary Output Files Ledger
    cur.execute(
        """
    ...
    # 11. Traceability & Version Mapping Ledger
    cur.execute(
        """
    ...
    # 12. Discrepancy & Variance Audit Ledger
    cur.execute(
        """
    ...
    if cur.fetchone()[0] == 0:
        ...
        cur.executemany(
            "INSERT INTO loading_bays (bay_no, bay_name, status) VALUES (?, ?, ?)",
            default_bays,
        )
...
def export_dataframe_to_excel_bytes(
    df: pd.DataFrame, sheet_name="DataSheet"
) -> bytes:
...
def build_pdf_loading_slip(trip_data: dict, items_df: pd.DataFrame) -> bytes:
    ...
    pdf.cell(
        190,
        5,
        f"Official Gate Pass & Loading Manifest | Generated (IST): {get_ist_timestamp_full()}",
        ln=True,
        align="C",
    )
    pdf.ln(4)
    ...
    pdf.cell(
        95, 6, f"Trip Date: {trip_data.get('trip_date', '')}", border=1, ln=True
    )
    pdf.cell(95, 6, f"Vehicle No: {trip_data.get('vehicle_no', '')}", border=1)
    pdf.cell(
        95, 6, f"Route No: {trip_data.get('route_no', '')}", border=1, ln=True
    )
    pdf.cell(
        95, 6, f"Transporter: {trip_data.get('transporter_name', '')}", border=1
    )
    pdf.cell(
        95,
        6,
        f"Loading Bay: {trip_data.get('loading_bay', '')}",
        border=1,
        ln=True,
    )
    pdf.cell(
        95, 6, f"Driver Name: {trip_data.get('driver_name', '')}", border=1
    )
    pdf.cell(
        95,
        6,
        f"Driver Phone: {trip_data.get('driver_phone', '')}",
        border=1,
        ln=True,
    )
    pdf.ln(4)
    ...
    for _, it in items_df.iterrows():
        ...
        pdf.cell(
            30,
            5,
            f"{float(it.get('allocated_bags', 0)):,.0f}",
            border=1,
            align="R",
        )
        pdf.cell(
            30,
            5,
            f"{float(it.get('allocated_weight_mt', 0)):,.2f}",
            border=1,
            ln=True,
            align="R",
        )
    ...
    pdf.cell(
        30,
        6,
        f"{float(trip_data.get('total_bags', 0)):,.0f}",
        border=1,
        align="R",
    )
    pdf.cell(
        30,
        6,
        f"{float(trip_data.get('total_weight_mt', 0)):,.2f}",
        border=1,
        ln=True,
        align="R",
    )
...
with st.sidebar:
    ...
    st.session_state.whatsapp_num = st.text_input(
        "Logistics Contact", value=st.session_state.whatsapp_num
    )
...
if main_menu == "⚡ Inbound Demand & Sales Order Engine":
    ...
    st.markdown(
        "Upload multiple **Demand Workbooks** to execute DR auto-lookup, eliminate duplicate route re-uploads, generate structured `Output.xlsx` files, and sync pending demand."
    )

    with st.expander(
        "⚙️ SKU, Route & Multi-Channel Dispatch Settings", expanded=False
    ):
        c1, c2, c3 = st.columns(3)
        with c1:
            st.session_state.fg_code = st.text_input(
                "Default Fallback FG Code", value=st.session_state.fg_code
            )
            st.session_state.route = st.text_input(
                "Default Route Fallback", value=st.session_state.route
            )
        with c2:
            st.session_state.col_map = st.text_area(
                "Column Index Mapping (Col:FG)",
                value=st.session_state.col_map,
                height=80,
            )
        with c3:
            st.session_state.agency_override = st.text_area(
                "Agency SKU Overrides (Agency:Col:FG)",
                value=st.session_state.agency_override,
                height=80,
            )
        ...
        c4, c5 = st.columns(2)
        with c4:
            st.session_state.email_user = st.text_input(
                "Sender Gmail ID", value=st.session_state.email_user
            )
            st.session_state.email_pass = st.text_input(
                "Gmail App Password",
                type="password",
                value=st.session_state.email_pass,
            )
        with c5:
            st.session_state.recipient = st.text_input(
                "Recipient Email", value=st.session_state.recipient
            )
    ...
    if uploaded_files and st.button(
        "🚀 Process Batch Orders & Ingest to Pending Database", type="primary"
    ):
        ...
        cur.execute(
            "SELECT DISTINCT route_no FROM pending_orders WHERE status='Pending'"
        )
        existing_pending_routes = set(str(r[0]).strip() for r in cur.fetchall())
        ...
        for up_file in uploaded_files:
            ...
            if fg_row == -1:
                st.warning(
                    f"⚠️ 'FG' column header missing in '{short_fname}'. Skipped."
                )
                continue

            # Detect Total Boundary
            total_col = df_input.shape[1]
            for c_s in range(fg_col, df_input.shape[1]):
                cell_v = str(df_input.iloc[fg_row, c_s]).strip().upper()
                if any(kw in cell_v for kw in ["TOTAL", "SUM", "NET"]):
                    total_col = c_s
                    break

            # Route Detection
            route_num = (
                st.session_state.route if st.session_state.route != "" else "22"
            )
            for r in range(fg_row):
                for c in range(min(total_col, 30)):
                    val = str(df_input.iloc[r, c]).strip()
                    if (
                        val != ""
                        and 1 <= len(val) <= 3
            ...
            if resolved_route in existing_pending_routes:
            ...
            for c_s in range(fg_col - 1, -1, -1):
                sample_v = str(
                    df_input.iloc[fg_row + 1, c_s]
                    if fg_row + 1 < df_input.shape[0]
                    else ""
                ).strip().upper()
                if re.match(r"^DR\d+", sample_v):
                    dr_code_col = c_s
                    break

            valid_cols = [
                (c, str(df_input.iloc[fg_row, c]).strip())
                for c in range(fg_col, total_col)
            ]

            wb_valid = (
                openpyxl.load_workbook(io.BytesIO(template_bytes))
                if template_bytes
                else openpyxl.Workbook()
            )
            ws_valid = (
                wb_valid["Order Data"]
                if "Order Data" in wb_valid.sheetnames
                else wb_valid.active
            )
            ...
            ws_missing = (
                wb_missing["Order Data"]
                if "Order Data" in wb_missing.sheetnames
                else wb_missing.active
            )
            ...
            for r in range(fg_row + 1, df_input.shape[0]):
                ...
                if (
                    pd.isna(agency_raw)
                    or str(agency_raw).strip() in ["", "nan", "None"]
                ):
                    continue
                ...
                if not clean_dr:
                    ...
                    if match:
                        ...
                        unmapped_records_to_insert.append(
                            (
                                short_fname,
                                resolved_route,
                                str(agency_val),
                        ...
                        st.session_state.unmapped_current_batch.append(
                            {
                                ...
                                "Agency": agency_val,
                                "Fallback DR": clean_dr,
                            }
                        )

                is_valid_dr = clean_dr.upper().startswith("DR")
                if is_valid_dr:
                    master_routes_to_insert.append(
                        (
                            short_fname,
                            resolved_route,
                            str(agency_val),
                    ...
                    agency_counts_valid[agency_val] = (
                        agency_counts_valid.get(agency_val, 0) + 1
                    )
                    seq_num = agency_counts_valid[agency_val]
                    ref_code = (
                        f"RT-{resolved_route}-{agency_val}-{today_date}"
                        if seq_num == 1
                        else f"RT-{resolved_route}-{agency_val}-{today_date}-{seq_num}"
                    )
                    target_ws, curr_r, order_id_to_write = (
                        ws_valid,
                        valid_r_idx,
                        valid_order_no,
                    )
                else:
                    agency_counts_missing[agency_val] = (
                        agency_counts_missing.get(agency_val, 0) + 1
                    )
                    seq_num = agency_counts_missing[agency_val]
                    ref_code = (
                        f"RT-{resolved_route}-{agency_val}-{today_date}-NEW"
                        if seq_num == 1
                        else f"RT-{resolved_route}-{agency_val}-{today_date}-NEW-{seq_num}"
                    )
                    target_ws, curr_r, order_id_to_write = (
                        ws_missing,
                        missing_r_idx,
                    ...
                    if pd.notna(q_val):
                        try:
                            ...
                            if f_qty > 0:
                                current_fg = (
                                    fg_val
                                    if fg_val.startswith("FG")
                                    else col_map_dict.get(
                                        c_idx, st.session_state.fg_code
                                    )
                                )
                                if (agency_val, c_idx) in agency_override_dict:
                                    current_fg = agency_override_dict[
                                        (agency_val, c_idx)
                                    ]
                                ...
                                target_ws.cell(
                                    row=curr_r,
                                    column=2,
                                    value=order_id_to_write,
                                )
                                target_ws.cell(
                                    row=curr_r, column=3, value="OR"
                                )
                                target_ws.cell(
                                    row=curr_r, column=4, value="SO20"
                                )
                                target_ws.cell(row=curr_r, column=5, value=10)
                                target_ws.cell(row=curr_r, column=6, value=20)
                                target_ws.cell(
                                    row=curr_r, column=7, value=clean_dr
                                )
                                target_ws.cell(
                                    row=curr_r, column=8, value=clean_dr
                                )
                                target_ws.cell(
                                    row=curr_r, column=9, value=ref_code
                                )
                                target_ws.cell(
                                    row=curr_r, column=10, value=today_date
                                )
                                target_ws.cell(
                                    row=curr_r, column=11, value=today_date
                                )
                                target_ws.cell(
                                    row=curr_r, column=15, value=item_seq_id
                                )
                                target_ws.cell(
                                    row=curr_r, column=16, value=current_fg
                                )
                                target_ws.cell(
                                    row=curr_r, column=19, value=f_qty
                                )
                                target_ws.cell(
                                    row=curr_r, column=20, value="Bag"
                                )
                                target_ws.cell(
                                    row=curr_r, column=22, value=2100
                                )
                                target_ws.cell(
                                    row=curr_r, column=26, value=resolved_route
                                )
                                target_ws.cell(
                                    row=curr_r, column=27, value=agency_val
                                )
                    ...
                    st.session_state.anomaly_logs.append(
                        {
                            ...
                            "Agency": agency_val,
                            "Qty": row_tot_bags,
                            "Alert": "High Volume Demand Spike (>500 Bags)",
                        }
                    )
            ...
            if valid_items_cnt > 0:
                ...
                st.session_state.processed_files.append(
                    {
                        "name": f"{short_fname} (Valid DR)",
                        "data": buf_v.getvalue(),
                        "filename": out_name_v,
                        "orders": valid_items_cnt,
                    }
                )
                cur.execute(
            ...
            if missing_items_cnt > 0:
                ...
                st.session_state.processed_files.append(
                    {
                        "name": f"{short_fname} (Missing DR)",
                        "data": buf_m.getvalue(),
                        "filename": out_name_m,
                        "orders": missing_items_cnt,
                    }
                )
                cur.execute(
            ...
            """,
                (
                    short_fname,
                    batch_ts,
                    valid_items_cnt + missing_items_cnt,
                    round(len(f_bytes) / 1024, 2),
                ),
            )
        ...
        cur.execute(
            "INSERT INTO history_logs (timestamp, files_count, total_qty, status) VALUES (?, ?, ?, 'Success')",
            (batch_ts, len(uploaded_files), total_in_qty),
        )
        ...
        st.success(
            f"🎉 Batch execution completed! Created {len(st.session_state.processed_files)} output workbooks & saved {len(pending_records_to_insert)} pending orders."
        )
        if skipped_dup_routes > 0:
            st.warning(
                f"⚠️ {skipped_dup_routes} Route(s) skipped because active pending records for those routes already exist."
            )

    # Multi-Channel Export & Notifications Panel
    if st.session_state.processed_files:
        st.markdown("---")
        st.subheader("📥 Multi-Channel Export, PDF Invoice & Notifications Hub")
        ...
        with c_exp1:
            st.download_button(
                "📦 Download ZIP",
                zip_buf.getvalue(),
                f"Batch_Orders_{get_ist_date_str()}.zip",
                "application/zip",
            )
        with c_exp2:
            try:
                ...
                pdf_rep.cell(
                    190,
                    10,
                    "Sales Orders Execution Invoice",
                    ln=True,
                    align="C",
                )
                ...
                pdf_rep.cell(
                    190,
                    6,
                    f"Generated On (IST): {get_ist_timestamp_full()}",
                    ln=True,
                    align="C",
                )
                pdf_rep.ln(6)
                ...
                st.download_button(
                    "📄 PDF Report",
                    bytes(pdf_rep.output()),
                    f"Execution_Invoice_{get_ist_date_str()}.pdf",
                    "application/pdf",
                )
            except Exception as e:
        ...
        with c_exp3:
            ...
            st.download_button(
                "📄 Summary TXT",
                txt_summary.encode("utf-8"),
                f"Summary_{get_ist_date_str()}.txt",
                "text/plain",
            )
        with c_exp4:
            json_dump = json.dumps(
                {"timestamp": get_ist_timestamp_full(), "kpi": kpi}, indent=4
            )
            st.download_button(
                "💾 Backup JSON",
                json_dump.encode("utf-8"),
                f"Audit_{get_ist_date_str()}.json",
                "application/json",
            )
        with c_exp5:
            components.html(
                """<button onclick="parent.window.print()" style="width:100%; height:38px; background:#2563eb; color:white; border:none; border-radius:4px; font-weight:600; cursor:pointer;">🖨️ Print View</button>""",
                height=45,
            )
        with c_exp6:
            if st.button("📧 Email Reports"):
                if (
                    ...
                    and st.session_state.email_pass
                    and st.session_state.recipient
                ):
                    try:
                        msg = EmailMessage()
                        msg["Subject"] = (
                            f"🚀 Sales Demand Execution Report - {get_ist_date_str()}"
                        )
                        msg["From"] = st.session_state.email_user
                        msg["To"] = st.session_state.recipient
                        msg.set_content(
                            f"Daily Demand Batch processed successfully on {get_ist_timestamp_full()}.\nTotal Bags: {kpi['input_qty']}"
                        )
                        for item in st.session_state.processed_files:
                            msg.add_attachment(
                                item["data"],
        ...
        with c_exp7:
            ...
            st.markdown(
                f'<a href="https://wa.me/{st.session_state.whatsapp_num}?text={urllib.parse.quote(wa_text)}" target="_blank"><button style="width:100%; height:38px; background:#25D366; color:white; border:none; border-radius:4px; font-weight:600;">📱 WhatsApp</button></a>',
                unsafe_allow_html=True,
            )
...
# ==============================================================================
    ...
    df_pending = pd.read_sql(
        "SELECT * FROM pending_orders WHERE status='Pending'", conn
    )

    if df_pending.empty:
        st.info(
            "ℹ️ No pending order demand found. Upload demand workbooks in Module 1."
        )
    else:
        route_summary = (
            df_pending.groupby("route_no")
            .agg({"agency_no": "nunique", "bags_qty": "sum", "weight_mt": "sum"})
            .reset_index()
            .rename(
                columns={
                    "agency_no": "Total Agencies",
                    "bags_qty": "Total Bags",
                    "weight_mt": "Total MT",
                }
            )
        ...
        with col_p1:
            sel_route = st.selectbox(
                "1. Select Route for Dispatch Trip",
                route_summary["route_no"].tolist(),
            )
            route_df = df_pending[df_pending["route_no"] == str(sel_route)]

            avail_fleet = pd.read_sql(
                "SELECT * FROM fleet_master WHERE status='Available'", conn
            )
            avail_bays = pd.read_sql(
                "SELECT * FROM loading_bays WHERE status='Open'", conn
            )

            fleet_opts = [
                f"{r['vehicle_no']} | {r['vehicle_type']} (Cap: {r['capacity_bags']} Bags / {r['capacity_mt']} MT)"
                for _, r in avail_fleet.iterrows()
            ]
            sel_vehicle = st.selectbox(
                "2. Assign Available Vehicle",
                fleet_opts if fleet_opts else ["No Vehicles Available"],
            )
            sel_bay = st.selectbox(
                "3. Assign Loading Bay",
                [
                    f"{r['bay_no']} - {r['bay_name']}"
                    for _, r in avail_bays.iterrows()
                ],
            )
        ...
        with col_p2:
            ...
            selected_agencies = st.multiselect(
                "Select Agencies", agencies, default=agencies
            )
            ...
            if sel_vehicle != "No Vehicles Available":
                ...
                util_pct = (
                    (trip_bags / cap_bags * 100) if cap_bags > 0 else 0.0
                )
                st.metric(
                    "Total Load Allocated",
                    f"{trip_bags:,.0f} / {cap_bags} Bags",
                    f"{util_pct:.1f}% Capacity Utilization",
                )

        st.markdown("---")
        if st.button(
            "🚀 Confirm Trip & Generate Loading Slip", type="primary"
        ):
            if sel_vehicle == "No Vehicles Available" or trip_df.empty:
                ...
                """,
                    (
                        ...
                        trip_bags,
                        trip_mt,
                        round((trip_bags / v_info["capacity_bags"] * 100), 2),
                ...
                for seq, (_, r_val) in enumerate(trip_df.iterrows(), 1):
                    ...
                    cur.execute(
                        "UPDATE pending_orders SET status='Assigned' WHERE id=?",
                        (r_val["id"],),
                    )

                cur.execute(
                    "UPDATE fleet_master SET status='Assigned to Trip' WHERE vehicle_no=?",
                    (v_num,),
                )
                conn.commit()
...
# ==============================================================================
    ...
    df_trips = pd.read_sql(
        "SELECT * FROM trip_loading_slips ORDER BY created_at DESC", conn
    )

    search_q = st.text_input(
        "🔍 Search Trips (Trip ID, Vehicle, Route, Transporter):", ""
    )
    if search_q:
        df_trips = df_trips[
            df_trips.apply(
    ...
    with c_t1:
        if not df_trips.empty:
            st.download_button(
                ...
                to_excel_download_bytes(df_trips, "Trips"),
                "Dispatch_Trips.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    with c_t2:
        with st.expander("🗑️ Multi-Select Delete Trips"):
            del_t_ids = st.multiselect(
                "Select Trip IDs to Delete:",
                df_trips["trip_id"].tolist() if not df_trips.empty else [],
            )
            if st.button("Delete Selected Trips"):
                cur = conn.cursor()
                for tid in del_t_ids:
                    v = cur.execute(
                        "SELECT vehicle_no FROM trip_loading_slips WHERE trip_id=?",
                        (tid,),
                    ).fetchone()
                    if v:
                        cur.execute(
                            "UPDATE fleet_master SET status='Available' WHERE vehicle_no=?",
                            (v[0],),
                        )
                    cur.execute(
                        "DELETE FROM trip_order_items WHERE trip_id=?", (tid,)
                    )
                    cur.execute(
                        "DELETE FROM trip_loading_slips WHERE trip_id=?", (tid,)
                    )
                conn.commit()
    ...
    if not df_trips.empty:
        ...
        items_df = pd.read_sql(
            "SELECT * FROM trip_order_items WHERE trip_id=? ORDER BY delivery_seq ASC",
            conn,
            params=(sel_trip,),
        )
        ...
        with btn_c1:
            ...
            st.download_button(
                "📄 Download PDF Loading Slip",
                pdf_slip,
                f"Loading_Slip_{sel_trip}.pdf",
                "application/pdf",
            )
        with btn_c2:
            ...
            st.markdown(
                f'<a href="https://wa.me/{st.session_state.whatsapp_num}?text={urllib.parse.quote(wa_text)}" target="_blank"><button style="width:100%; height:38px; background:#25D366; color:white; border:none; border-radius:4px; font-weight:600;">📱 WhatsApp Alert</button></a>',
                unsafe_allow_html=True,
            )
        with btn_c3:
            if trip_row["status"] != "Dispatched" and st.button(
                "🏁 Gate Out / Dispatched", type="primary"
            ):
                cur = conn.cursor()
                for _, it in items_df.iterrows():
                    cur.execute(
                    ...
                    """,
                        (
                            ...
                            it["fg_code"],
                            it["allocated_bags"],
                            it["allocated_weight_mt"],
                            trip_row["loading_bay"],
                            get_ist_timestamp_full(),
                        ),
                    )
                cur.execute(
                    "UPDATE trip_loading_slips SET status='Dispatched' WHERE trip_id=?",
                    (sel_trip,),
                )
                cur.execute(
                    "UPDATE fleet_master SET status='Available' WHERE vehicle_no=?",
                ...
                conn.commit()
                st.success(
                    "✅ Dispatched & recorded in Daily Dispatch Register!"
                )
                st.rerun()
    conn.close()
...
# ==============================================================================
    ...
    conn = get_db_connection()
    df_reg = pd.read_sql(
        "SELECT * FROM daily_dispatch_register ORDER BY register_id DESC", conn
    )

    search_r = st.text_input(
        "🔍 Search Register (Agency, Route, Vehicle, DR Code, Order No):", ""
    )
    if search_r:
        df_reg = df_reg[
            df_reg.apply(
                lambda r: r.astype(str).str.contains(search_r, case=False).any(),
                axis=1,
            )
        ]
    ...
    with c1:
        if not df_reg.empty:
            st.download_button(
                ...
                to_excel_download_bytes(df_reg, "DailyRegister"),
                "Daily_Dispatch_Sale_Register.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    with c2:
        with st.expander("🗑️ Multi-Select Delete Register Records"):
            del_r_ids = st.multiselect(
                "Select Register IDs to Delete:",
                df_reg["register_id"].tolist() if not df_reg.empty else [],
            )
            if st.button("Delete Selected Records"):
                cur = conn.cursor()
                cur.executemany(
                    "DELETE FROM daily_dispatch_register WHERE register_id=?",
                    [(i,) for i in del_r_ids],
                )
                conn.commit()
...
# ==============================================================================
    ...
    search_p = st.text_input(
        "🔍 Search Pending Orders (Order No, Route, Agency, FG Code, DR Code):",
        "",
    )
    if search_p:
        df_p = df_p[
            df_p.apply(
                lambda r: r.astype(str).str.contains(search_p, case=False).any(),
                axis=1,
            )
        ]
    ...
    with c1:
        if not df_p.empty:
            st.download_button(
                ...
                to_excel_download_bytes(df_p, "PendingOrders"),
                "Pending_Orders.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    with c2:
        with st.expander("🗑️ Multi-Select Delete Pending Orders"):
            del_p_ids = st.multiselect(
                "Select Order IDs to Delete:",
...
# ==============================================================================
    ...
    conn = get_db_connection()
    df_a = pd.read_sql(
        "SELECT * FROM uploaded_files_archive ORDER BY id DESC", conn
    )
    ...
    if search_a:
        df_a = df_a[
            df_a.apply(
                lambda r: r.astype(str).str.contains(search_a, case=False).any(),
                axis=1,
            )
        ]
    ...
    with c1:
        if not df_a.empty:
            st.download_button(
                ...
                to_excel_download_bytes(df_a, "ArchiveLogs"),
                "Uploaded_Archive_Logs.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
    with c2:
        with st.expander("🗑️ Multi-Select Delete Archive Logs"):
            del_a_ids = st.multiselect(
                "Select File IDs:",
                df_a["id"].tolist() if not df_a.empty else [],
            )
            if st.button("Delete Selected Logs"):
                cur = conn.cursor()
                cur.executemany(
                    "DELETE FROM uploaded_files_archive WHERE id=?",
                    [(i,) for i in del_a_ids],
                )
                conn.commit()
...
# ==============================================================================
    ...
    conn = get_db_connection()
    df_m = pd.read_sql(
        "SELECT * FROM unique_routes_master ORDER BY id DESC", conn
    )
    df_u = pd.read_sql(
        "SELECT * FROM unmapped_missing_dr_ledger ORDER BY id DESC", conn
    )

    t1, t2 = st.tabs(
        ["📋 Unique Master Mapping DB", "🚨 Unmapped Missing DR Ledger"]
    )
    ...
    with t1:
        ...
        if sm:
            df_m = df_m[
                df_m.apply(
                    lambda r: r.astype(str).str.contains(sm, case=False).any(),
                    axis=1,
                )
            ]
        ...
        with c1:
            st.download_button(
                ...
                to_excel_download_bytes(df_m, "MasterRoutes"),
                "Unique_Routes_Master.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        with c2:
            with st.expander("🗑️ Multi-Select Delete Master Records"):
                del_m = st.multiselect(
                    "Select IDs to Delete:",
                    df_m["id"].tolist() if not df_m.empty else [],
                )
                if st.button("Delete Master Records"):
                    cur = conn.cursor()
                    cur.executemany(
                        "DELETE FROM unique_routes_master WHERE id=?",
                        [(i,) for i in del_m],
                    )
                    conn.commit()
        ...
        with st.expander("➕ Add Single Record to Master DB"):
            ...
            with m_c3:
                man_dr = st.text_input("DR Code", "")
            if st.button("Save Master Record"):
                if man_r and man_a and man_dr:
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT OR REPLACE INTO unique_routes_master (file_name, route_no, agency_no, dr_code, created_at) VALUES ('Manual_Entry', ?, ?, ?, ?)",
    ...
    with t2:
        ...
        if su:
            df_u = df_u[
                df_u.apply(
                    lambda r: r.astype(str).str.contains(su, case=False).any(),
                    axis=1,
                )
            ]
        ...
        with c1:
            st.download_button(
                "📥 Export Unmapped to Excel",
                to_excel_download_bytes(df_u, "UnmappedDR"),
                "Unmapped_Missing_DR.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        with c2:
            with st.expander("🗑️ Multi-Select Delete Unmapped"):
                del_u = st.multiselect(
                    "Select Unmapped IDs:",
                    df_u["id"].tolist() if not df_u.empty else [],
                )
                if st.button("Delete Unmapped Records"):
                    cur = conn.cursor()
                    cur.executemany(
                        "DELETE FROM unmapped_missing_dr_ledger WHERE id=?",
                        [(i,) for i in del_u],
                    )
                    conn.commit()
    ...
    with tab1:
        ...
        with c1:
            st.download_button(
                ...
                to_excel_download_bytes(df_f, "Fleet"),
                "Fleet_Master.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        with c2:
            with st.expander("🗑️ Delete Vehicles"):
                del_f = st.multiselect(
                    "Select Vehicle IDs:",
                    df_f["id"].tolist() if not df_f.empty else [],
                )
                if st.button("Delete Vehicles"):
                    cur = conn.cursor()
                    cur.executemany(
                        "DELETE FROM fleet_master WHERE id=?",
                        [(i,) for i in del_f],
                    )
                    conn.commit()
                    st.rerun()
        ...
        with st.expander("➕ Add Vehicle"):
            ...
            with v2:
                v_bags = st.number_input(
                    "Capacity (Bags)", value=500, step=50
                )
            ...
            if st.button("Save Vehicle"):
                if v_num:
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT OR REPLACE INTO fleet_master (vehicle_no, vehicle_type, capacity_bags, capacity_mt, transporter_name, driver_name, driver_phone, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'Available')",
                        (
                            v_num,
                            v_type,
                            v_bags,
    ...
    with tab2:
        ...
        with c1:
            st.download_button(
                ...
                to_excel_download_bytes(df_b, "Bays"),
                "Loading_Bays.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        with c2:
            with st.expander("🗑️ Delete Bays"):
                del_b = st.multiselect(
                    "Select Bay IDs:",
                    df_b["id"].tolist() if not df_b.empty else [],
                )
                if st.button("Delete Bays"):
                    cur = conn.cursor()
                    cur.executemany(
                        "DELETE FROM loading_bays WHERE id=?",
                        [(i,) for i in del_b],
                    )
                    conn.commit()
                    st.rerun()
        ...
        with st.expander("➕ Add Bay"):
            b1, b2 = st.columns(2)
            with b1:
                bay_no = st.text_input("Bay Code (e.g. BAY-04)")
            with b2:
                bay_name = st.text_input("Location Name")
            if st.button("Save Bay"):
                if bay_no:
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT OR REPLACE INTO loading_bays (bay_no, bay_name, status) VALUES (?, ?, 'Open')",
                        (bay_no, bay_name),
                    )
                    conn.commit()
                    st.rerun()
...
# ==============================================================================
    ...
    conn = get_db_connection()
    df_trace = pd.read_sql(
        "SELECT id, batch_timestamp, input_file_name, total_input_qty, generated_output_file, output_type, version_no FROM input_output_traceability ORDER BY id DESC",
        conn,
    )
    df_stored = pd.read_sql(
        "SELECT id, file_name, file_type, created_at FROM output_files_ledger ORDER BY id DESC",
        conn,
    )
    df_disc = pd.read_sql(
        "SELECT * FROM discrepancy_audit_ledger ORDER BY id DESC", conn
    )
    ...
    with t_tab1:
        ...
        if not df_trace.empty:
            st.download_button(
                ...
                to_excel_download_bytes(df_trace, "Traceability"),
                "Traceability_Ledger.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
            with st.expander("🗑️ Multi-Delete Traceability"):
                ...
                if st.button("Delete Traceability"):
                    cur = conn.cursor()
                    cur.executemany(
                        "DELETE FROM input_output_traceability WHERE id=?",
                        [(i,) for i in del_tr],
                    )
                    conn.commit()
                    st.rerun()
    ...
    with t_tab2:
        ...
        if not df_stored.empty:
            sel_out_id = st.number_input(
                "Enter Stored Output ID to Download", min_value=1, step=1
            )
            if st.button("Download Stored File"):
                cur = conn.cursor()
                res = cur.execute(
                    "SELECT file_name, file_data FROM output_files_ledger WHERE id=?",
                    (sel_out_id,),
                ).fetchone()
                if res:
                    st.download_button(
...
# ==============================================================================
    ...
    tot_dispatched_bags = (
        df_reg["dispatched_bags"].sum() if not df_reg.empty else 0.0
    )
    avg_util = (
        df_trips["capacity_utilization_pct"].mean()
        if not df_trips.empty
        else 0.0
    )
    active_pending_bags = (
        df_pending[df_pending["status"] == "Pending"]["bags_qty"].sum()
    ...
    with c_ch2:
        ...
        if not df_reg.empty:
            st.bar_chart(
                df_reg.groupby("transporter_name")["dispatched_bags"].sum()
            )
        else:
