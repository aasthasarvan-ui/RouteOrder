...
# 🌟 BACKGROUND AUTO-MAPPING HOOK (UPLOAD HOTE HI RUN HOGA)
if uploaded_file is not None:
    try:
        # 1. Master Mapping Load karo
        mapping_dict_bg = {}
        ...
        # 2. Raw File Read karke FG aur Agency detect karo
        raw_bytes = uploaded_file.getvalue()
        df_raw_bg = pd.read_excel(io.BytesIO(raw_bytes), header=None)
        
        f_row, f_col = -1, -1
        for r in range(df_raw_bg.shape[0]):
            for c in range(df_raw_bg.shape[1]):
                if "FG" in str(df_raw_bg.iloc[r, c]).strip().upper():
                    f_row, f_col = r, c
                    break
            if f_row != -1: break
        ...
        if f_row != -1:
            rt_num_bg = "22"
            m_rt_bg = re.search(r'Route\s*\(?(\d+)\)?', uploaded_file.name, re.IGNORECASE)
            if m_rt_bg: rt_num_bg = m_rt_bg.group(1)

            ag_col_bg = -1
            for c_s in range(f_col - 1, -1, -1):
                v_cnt = 0
                for r_c in range(f_row + 1, min(f_row + 15, df_raw_bg.shape[0])):
                    val_chk = df_raw_bg.iloc[r_c, c_s]
                    if pd.notna(val_chk) and str(val_chk).replace('.0', '').strip().isdigit():
                        v_cnt += 1
                if v_cnt >= 3:
                    ag_col_bg = c_s
                    break
            if ag_col_bg == -1 and f_col > 0: ag_col_bg = f_col - 1

            # 3. Openpyxl se DRCODE column insert karo aur formulas shift karo
            wb_bg = openpyxl.load_workbook(io.BytesIO(raw_bytes))
            ws_bg = wb_bg.active
            ...
            has_dr = False
            for ci in range(1, ws_bg.max_column + 1):
                if str(ws_bg.cell(row=ex_f_row, column=ci).value).strip().upper() == "DRCODE":
                    has_dr = True
                    t_idx = ci
                    break
            
            if not has_dr:
                ws_bg.insert_cols(ex_f_col)
                t_idx = ex_f_col
                ws_bg.cell(row=ex_f_row, column=t_idx, value="DRCODE")
                
                # Formula shifting
                for row_cells in ws_bg.iter_rows():
            ...
            # 4. Master se DRCODE values fill karo
            for r_idx in range(ex_f_row + 1, ws_bg.max_row + 1):
                raw_ag = ws_bg.cell(row=r_idx, column=ag_col_bg + 1).value
                ag_str = str(raw_ag).replace('.0', '').strip() if raw_ag is not None else ""
                if ag_str and ag_str != "None":
                    k = (str(rt_num_bg), ag_str)
                    assigned = mapping_dict_bg.get(k, f"NEW_CUST_{ag_str}")
                    ws_bg.cell(row=r_idx, column=t_idx, value=assigned)

            # 5. Modified file ko wahi 'uploaded_file' variable bana do taki baki ka poora code isi updated file ko read kare
            mod_buf = io.BytesIO()
            wb_bg.save(mod_buf)
            mod_buf.seek(0)
            uploaded_file = mod_buf
    except Exception as e:
        pass
