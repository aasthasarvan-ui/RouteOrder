...
def verify_core_integrity():
    try:
        ...
        required_tables = [
            ...
            'discrepancy_audit_ledger', 'dispatch_planning_ledger'
        ]
...
# ==========================================
# 4. CONTROL PANEL & CONFIGURATION INTERFACE
# ==========================================
            ...
            try:
                ...
                for uploaded_file in uploaded_inputs:
                    ...
                    for r in range(fg_row + 1, df_input.shape[0]):
                        ...
                        # Vehicle Capacity Load Splitting & Shortfall Pending Logic
                        ...
                        for c, fg_code, qty_val in valid_row_quantities:
                            ...
                            diff_val = qty_val - dispatched_val
                            if diff_val != 0:
                                discrepancy_records.append((batch_ts, short_filename, str(agency_val), dr_to_use, current_fg, qty_val, dispatched_val, diff_val, batch_ts))
                ...
                if dispatch_plan_records:
                    cursor.executemany("""
                        INSERT INTO dispatch_planning_ledger (dispatch_date, route_no, vehicle_no, driver_mobile, agency_no, demand_qty, dispatched_qty, pending_qty, status, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, dispatch_plan_records)
